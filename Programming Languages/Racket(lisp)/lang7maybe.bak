...
(define (parse concrete)
  (cond
    ...
    [(equal? (first concrete) 'DASBINDEN)
     (binden (parse(first (second concrete))) (parse(second (second concrete))) (parse(rest(rest concrete))))]
...
;; CONTRACT
;; einfügen is german for insert.
;; symbol value environment -> environment (which includeds (tuple symbol value))
(define (einfügen id val enviro)
  (cons (bound id val) enviro))
;; I'm a bit confused...

;; CONTRACT
;; nachschlagen is german for lookup.
;; symbol environment -> symbol
(define (nachschlagen id enviro)
  (cond
    [(empty? enviro)
     (error 'lookup "No value found for ~a" id)]
    [(equal? (bound-id (first enviro)) id)
     (bound-val (first enviro))]
    [else
     (nachschlagen id (rest enviro))]
    )
  )
...
(check-expect (empty-enviro) '())
(check-expect (insert 'x 5 (empty-env))
              (list (tuple 'x 5)))
...
;; random notes on functions from 4/17
;; Formal Parameter: shows up at definition
;; functions inside of bind evaluated at time of binding

;; What happens at the time of 1. parsing, 2. binding, and 3. evaluation?
;;
;; 1. Parsing - we would take the formal body and name and parse them so that they move into an abstract structure
;; 2. Binding - extend the environment with the ID and the bound expression. Then ask matt wtf is going on.
;; 3. Evaluation - interpret the final equation